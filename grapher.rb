...
dates.each do |date|
  if on?(date)
    5.times { |i| commit_dates << date.to_time + i * 3600 }
  end
end
...
ignore_dates = [
  Date.new(2015, 3, 8).to_time,
]

commit_dates.each do |date|
  # next unless date - Date.new(2014, 12, 23).to_time >= 0 && date - Date.new(2015, 1, 15).to_time < 0 #ignore_dates.include?(date)
